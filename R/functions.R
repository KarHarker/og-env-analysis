...
protected_area_load <- function(){
  pa_tp <- st_read("og-data/ShapeFile/ProtectedForest_2021_08_10.shp", crs=3005) %>%
  st_make_valid() %>%
  st_cast(to = "POLYGON", warn = FALSE)
  pa_tp
}
...
ch_data_flat <- function(){
  ch <- st_read("CriticalHabitat.gdb", layer='critical_flat', crs=3005)
...
get_water_licenses <- function(){
  water_lice <- bcdc_get_data("WHSE_WATER_MANAGEMENT.WLS_WATER_RIGHTS_LICENCES_SV") %>%
    rename_all(tolower) %>%
    st_make_valid() %>%
    filter(pod_status=="Active", licence_status=="Current")
}

bec_data_load <- function(){
...
df_leading <- function(){
  df<-read_stars("data/DougFir_Leading.tif")
  df_leading <- st_as_sf(df, merge=TRUE, long=TRUE) %>%
    filter(DougFir_Leading.tif == 1) %>%
    st_make_valid()%>%
    st_cast(to = "POLYGON", warn = FALSE)
}

df_second <- function(){
  df2<-read_stars("data/DougFir_Second.tif")
  df_second <- st_as_sf(df2, merge=TRUE, long=TRUE) %>%
    filter(DougFir_Second.tif == 1) %>%
    st_make_valid()%>%
    st_cast(to = "POLYGON", warn = FALSE)
}
...
fn_load <- function(){
  fn <-st_read("data/BCGWexports.gdb", layer="FirstNationCommunityLocations") %>%
    rename_all(tolower) %>%
    st_make_valid()
}

pop_load <- function(){
  pop<-st_read("data/StatsCan_populationCentersBCalbers.shp") %>%
    rename_all(tolower) %>%
    st_make_valid() %>%
    st_cast(to = "POLYGON", warn = FALSE)
}

bgc_at_risk<- function(){
...
woodlands <- function(){
  woodland <- st_read("data/og-oct-13/Shapefile/swb_woodland_2021_10_05.shp", crs=3005) %>%
    rename_all(tolower) %>%
    st_make_valid() %>%
    st_cast(to = "POLYGON", warn = FALSE)
  woodland
...
caribou_herds <- function(){
  caribou_area <- st_read("out/caribou_herds_gcr.gpkg", layer = 'caribou_herds_gcr', crs=3005)

  caribou_area <- caribou_area %>%
    rename_all(tolower) %>%
    st_zm() %>%
    st_cast(to = "MULTIPOLYGON", warn = FALSE) %>%
    st_make_valid() %>%
...
heat_map_26 <- function(){
  heat <- st_read("data/heatmap_og26_vect_11-03.gpkg", crs=3005) %>%
    st_make_valid() %>%
    st_cast(to = "POLYGON", warn = FALSE)
  heat
}
...
caribou_full<- function(){
herd_full <- st_read("out/caribou_herds_full.gpkg", crs=3005) %>%
  st_zm() %>%
  st_make_valid() %>%
  st_cast(to = "POLYGON", warn = FALSE)
}
...
y2y_area<- function(){
  area <- st_read("out/y2y_area.gpkg", crs=3005) %>%
    st_make_valid() %>%
    st_cast(to = "POLYGON", warn = FALSE)
  area
}
...
pip_areas<- function(){
  area <- st_read("data/ConsultationAreas.gdb", layer="PIP_CONSULTATION_AREAS_SP", crs=3005) %>%
    st_make_valid() %>%
    st_cast(to = "POLYGON", warn = FALSE)
  area
}
